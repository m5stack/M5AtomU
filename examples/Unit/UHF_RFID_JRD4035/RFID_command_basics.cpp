...
UDOUBLE UHF_RFID::String_to_command_frame(
    String str_epc)  // EPC string to command frame EPC字符串转命令帧
{
    UDOUBLE b;
...
void UHF_RFID::Warningmessage(UBYTE nub) {
    switch (nub) {
        ...
        case 0x14:
            DATA_Str_Serial =
                "BlockPermalock execution failed.The tag does not return or "
                "returns a CRC check of the data Error.";
        ...
        case 0x1A:
            DATA_Str_Serial =
                "The changeConfig directive failed. The tag does not return "
                "data or a return number Error check against CRC.";
            // ChangeConfig 指令失败，标签没有返回数据或者返回数据 CRC
            // 校验错误。
            break;
        case 0x2A:
            DATA_Str_Serial =
                "The ReadProtect instruction failed, the label does not return "
                "data or returns data CRC check error.";
            // ReadProtect 指令失败，标签没有返回数据或者返回数据CRC 校验错误。
            break;
        case 0x2B:
            DATA_Str_Serial =
                "Reset ReadProtect instruction failed, the label did not "
                "return data or returnBack data CRC check error.";
            // Reset ReadProtect 指令失败，标签没有返回数据或者返回数据 CRC
            // 校验错误。
            break;
        case 0x1B:
            DATA_Str_Serial =
                "The CHANGE EAS directive failed, and the label did not return "
                "data or returned data CRC check error.";
            // Change EAS 指令失败，标签没有返回数据或者返回数据CRC 校验错误。
            break;
        case 0x1D:
            DATA_Str_Serial =
                "The EAS_ALARM instruction failed with no label returning the "
                "correct ALARM CODE.";
            // EAS_Alarm 指令失败，没有标签返回正确 Alarm Code。
            break;
        case 0x2E:
            DATA_Str_Serial =
                "Qt instruction failed, tag did not return data or returned "
                "data CRC calibration Check error.";
...
void UHF_RFID::Copy_command_library(UBYTE com_nub) {
    while (RFID_cmdnub[com_nub][DATA_Interim_b] !=
           0x7E)  // First copy the command library 先复制命令库
    {
        DATA_Interim_order[DATA_Interim_b] =
            RFID_cmdnub[com_nub][DATA_Interim_b];
        DATA_Interim_b++;
    }
...
void UHF_RFID::Check_bit_accumulation() {
    DATA_Interim_order[DATA_Interim_b - 1] =
        0x00;  // Check bit accumulation 校验位累加

    for (UBYTE c = 1; c < DATA_Interim_b - 1; c++) {
        DATA_Interim_order[DATA_Interim_b - 1] =
            DATA_Interim_order[c] + DATA_Interim_order[DATA_Interim_b - 1];
    }
    if (DATA_Interim_order[DATA_Interim_b - 1] > 0xFF) {
        DATA_Interim_order[DATA_Interim_b - 1] =
            DATA_Interim_order[DATA_Interim_b - 1] % 256;
    }
}
...
UWORD UHF_RFID::ToHex(UDOUBLE parameters, UBYTE MSB, UBYTE LSB) {
    ...
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    // DATA_Interim_order[3] = 0x12;             //Split the reshape to a
    // command frame parameter 拆分整形装换为命令帧参数 DATA_Interim_order[4] =
    // 0x34;
...
UBYTE UHF_RFID::Verify_the_return(
    UBYTE a[], UBYTE size_v)  // Verify return information 验证返回信息
{
    UBYTE c = 0;
 ...
 *  Used to transfer string EPC to command frame
  ...
  用于将字符串EPC转命令帧
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
UBYTE UHF_RFID::EPC_string_to_command_frame(String str_epc, UBYTE MSB,
                                            UBYTE LSB) {
    UBYTE b = 0;
    b       = LSB - MSB;
