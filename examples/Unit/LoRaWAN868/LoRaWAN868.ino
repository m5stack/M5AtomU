...
information：https://docs.m5stack.com/en/unit/lorawan868
* 获取更多资料请访问：https://docs.m5stack.com/zh_CN/unit/lorawan868
*
* describe: LoRaWAN868.
* date：2021/8/31
*******************************************************************************
  Please connect to Port,请连接端口
*/
#include "M5Atom.h"
...
void loop() {
    ...
    if (recvStr.indexOf("+CJOIN:") != -1) {
        ...
        if (system_fsm == kJoined) {
            ...
            //{
            //    loraWanupLinkReviceCNT ++;
            //    sprintf(strbuff,"SEND CNT: %d
            //    (%d/%d)",loraWanSendCNT,loraWanupLinkReviceCNT,loraWanupLinkCNT);
            ...
            // else
            //{
            //    sprintf(strbuff,"FAILD NUM:%d CNT:%d
            //    (%d/%d)",loraWanSendNUM,loraWanSendCNT,loraWanupLinkReviceCNT,loraWanupLinkCNT);
            //    M5.Lcd.fillRect(0,183,320,32,TFT_BLACK);
        ...
        sprintf(strbuff, "ERROR Code:%d (%d/%d)", ErrorCodeStr.toInt(),
                loraWanupLinkReviceCNT, loraWanupLinkCNT);
        ...
        char strbuff[128];
        sprintf(strbuff, "%s (%d/%d)", checkStr.c_str(), loraWanupLinkReviceCNT,
                loraWanupLinkCNT);
        Serial.println(strbuff);
    }
